...
#if (NET_ERR_CFG_ARG_CHK_DBG_EN == DEF_ENABLED)
    switch (popt_route->Type) {                                 /* ----------------- VALIDATE OPT TYPE ---------------- */
...
#if (NET_ERR_CFG_ARG_CHK_DBG_EN == DEF_ENABLED)
    switch (popt_ts->Type) {                                    /* ----------------- VALIDATE OPT TYPE ---------------- */
                           ...
                           NET_ERR      *perr)
{
    ...
    CPU_BOOLEAN      ip_tx_frag;
                                                                ...
                                                                /* ----------------- PREPARE IP OPTS ------------------ */
    if (popts != (void *)0) {
        ip_opt_len_size = NetIP_TxPktPrepareOpt((NET_BUF     *) pbuf,
                                                (NET_BUF_HDR *) pbuf_hdr,
                                                (void        *) popts,
...
*               --------    Argument validated in NetIP_Tx().
*
*               popts       Pointer to one or more IP options configuration data structures (see Note #2) :
*               -----
*                               NULL                            NO IP transmit options configuration.
*                               NET_IP_OPT_CFG_ROUTE_TS         Route &/or Internet Timestamp options configuration.
*                               NET_IP_OPT_CFG_SECURITY         Security options configuration
