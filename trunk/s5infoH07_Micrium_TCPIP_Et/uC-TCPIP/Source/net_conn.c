...
*
*               (3) #### Return value may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
/*$PAGE*/
...
* Note(s)     : (1) Network connection addresses maintained in network-order.
*
*               (2) #### Return value may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
/*$PAGE*/
...
*               perr        Pointer to variable that will receive the return error code from this function :
*
*                               NET_CONN_ERR_NONE                   Remote address compared to network
                                    ...
                                    NET_ERR            *perr)
{
    ...
    pconn = &NetConn_Tbl[conn_id];
                                                                ...
                                                                /* ---------------- VALIDATE CONN USED ---------------- */
    used  =  DEF_BIT_IS_SET(pconn->Flags, NET_CONN_FLAG_USED);
    if (used != DEF_YES) {                                      /* If conn NOT used, rtn err.                           */
        NET_CTR_ERR_INC(NetConn_ErrNotUsedCtr);
       *perr =  NET_CONN_ERR_NOT_USED;
        return (DEF_NO);
    }
...
*
* Return(s)   : DEF_YES, connection   valid &      in use.
*
*               DEF_NO,  connection invalid or NOT in use.
*
* Caller(s)   : various.
*
*               This function is an INTERNAL network protocol suite function & SHOULD NOT be called by 
...
*                               NET_CONN_ERR_INVALID_LIST_TYPE      Invalid connection list type.
*
...
*               application function(s).
*
* Note(s)     : (1) Default case already invalidated in earlier connection functions.  However, the default 
*                   case is included as an extra precaution in case 'ConnListType' is incorrectly modified.
*
*               (2) #### 'perr' may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
/*$PAGE*/
...
*                               NET_CONN_ERR_INVALID_FAMILY             Invalid connection family.
*                               NET_CONN_ERR_INVALID_CONN               Invalid connection number(s).
