...
#define  TTCP_REMOTE_IP                             "0.0.0.0"   /* Remote IP address.                                   */
#define  TTCP_BUF_LEN                                   8192    /* Buffer length.                                       */
...
#define  TTCP_FMT                                         'k'   /* Output format: K = kilobits.                         */
...
*/
    ...
    CPU_CHAR           TTCP_RemoteIP[15];                       /* Remote host IP address.                              */
    CPU_INT16U         TTCP_BufLen;                             /* Length of buffer to use for TTCP.                    */
    CPU_CHAR           TTCP_Buf[65535];                         /* Buffer used for TTCP, maximum size of 64K.           */
    ...
    CPU_FP64           TTCP_NumBytes;                           /* Number of bytes carried on the network.              */
                                                                
    CPU_INT32U         TTCP_NumIntvl[TTCP_MAX_CONNS];           /* Number of interval bytes sent.                       */
    CPU_FP64           TTCP_NumBytesStream[TTCP_MAX_CONNS];     /* Number of interval bytes sent.                       */
...
static  void         TTCP_Pattern      (CPU_CHAR    *cp,
                                        CPU_INT16U   cnt);
static  void         TTCP_OutFmt       (CPU_FP64     b);
...
void  TTCP_Task (void  *p_arg)
{
    ...
    CPU_FP64           TTCP_PerfTimerSecs;                      /* Timer used to calculate the performance statisitics  */
                                                                /* in seconds.                                          */
    CPU_FP64           TTCP_BytesPerSec;
    CPU_FP64           TTCP_BitsPerSec;
    CPU_FP64           TTCP_CallsPerSec;
    CPU_FP64           TTCP_msPerCall;
    ...
    while (DEF_TRUE) {
        ...
        if (TTCP_LoopMode == 0) {
            while (TTCP_ParseErr) {
                ...
                TTCP_BufLen      = TTCP_BUF_LEN;                /* Buffer size.                                         */
                ...
                TTCP_TRACE_INFO(("uC/TCP-IP Performance measurements\r\n"));
                TTCP_TRACE_INFO(("Type the TTCP parameters, parameter values and press Enter\r\n"));
                TTCP_TRACE_INFO((">"));
        ...
        if (TTCP_Cli_Srvr) {                                    /* Output test parameters to serial port.               */
            TTCP_TRACE_INFO(("ttcp-t: BufLen=%d, NumBuf=%d, port=%d, NumConns=%d",
                              TTCP_BufLen, TTCP_NumBuf, TTCP_Port, TTCP_NumConns));
            if (TTCP_SockBufSize) {
                TTCP_TRACE_INFO((", sockbufsize=%d", TTCP_SockBufSize));
            }
            TTCP_TRACE_INFO(("  %s  -> %s\r\n", TTCP_Transport ? "udp" : "tcp", TTCP_RemoteIP));

        } else {
            TTCP_TRACE_INFO(("ttcp-r: BufLen=%d, NumBuf=%d, port=%d, NumConns=%d",
                              TTCP_BufLen, TTCP_NumBuf, TTCP_Port, TTCP_NumConns));
        ...
        TTCP_TRACE_INFO(("TTCP_RemoteIP = %s, TTCP_RemoteAddr = %x\n\r",
        ...
        if (TTCP_Cli_Srvr) {                                    /* Client mode.  Acts as a Transmitter.                 */
            ...
            for (i = 0; i < TTCP_NumConns; i++) {
                ...
                if (TTCP_ConnSockID[i] > -1) {                  /* Socket opened succesfully.                           */
                                                                /* Set blocking timeout until we 'connect'.             */
                    NetOS_Sock_ConnReqTimeoutSet(TTCP_ConnSockID[i], 6 * DEF_TIME_NBR_mS_PER_SEC, &TTCP_Err);

                    TTCP_TRACE_INFO(("ttcp%s: Client socket %d opened\r\n",
                                      TTCP_Cli_Srvr?"-t":"-r", TTCP_ConnSockID[i]));

                    TTCP_TRACE_INFO(("Connecting to addr: %x, port: %x\r\n",
                    ...
                    if (TTCP_RtnCode != NET_SOCK_BSD_ERR_CONN) {
                        TTCP_TRACE_INFO(("ttcp%s: Client socket %d connected\r\n",
                                          TTCP_Cli_Srvr ? "-t" : "-r", TTCP_ConnSockID[i]));
                    } else {
                        TTCP_TRACE_INFO(("ttcp%s: Unable to connect Client socket. Server may not be running. Error: %d\r\n",
                                          TTCP_Cli_Srvr ? "-t" : "-r", TTCP_Err));
                        NetSock_Close(TTCP_ConnSockID[i], &TTCP_Err);
                        TTCP_ConnSockID[i] = -1;
                    }
                } else {
                    TTCP_TRACE_INFO(("ttcp%s: Unable to open socket.\r\n",
                                      TTCP_Cli_Srvr ? "-t" : "-r"));
                }
            }
            ...
            if (TTCP_Transport) {                               /* Layer 4 protocol is UDP.                             */
               ...
               TTCP_TRACE_INFO(("ttcp%s: Waiting for client to send UDP datagrams.\r\n",
                                 TTCP_Cli_Srvr?"-t":"-r"));

                                                                /* Set blocking timeout to infinite until the Client    */
                                                                /* connects.                                            */
               NetOS_Sock_RxQ_TimeoutSet(TTCP_ConnSockID[i], NET_TMR_TIME_INFINITE, &TTCP_Err);

            } else {                                            /* Layer 4 protocol is TCP.                             */
                TTCP_ListenSockID = TTCP_TaskTCP_Init();        /* Open a socket to listen for incoming connections.    */

                TTCP_TRACE_INFO(("ttcp%s: Waiting for %d client to request connection.\r\n",
                                  TTCP_Cli_Srvr ? "-t" : "-r", TTCP_NumConns));

                TTCP_TRACE_INFO(("TTCP_RemoteAddLen = %d, NET_SOCK_ADDR_SIZE=%d.\r\n",
                ...
                while (i < TTCP_NumConns) {
                    ...
                    if (TTCP_ConnSockID[i] >= 0) {
                        TTCP_TRACE_INFO(("ttcp%s: Server socket %d active \r\n",
                                          TTCP_Cli_Srvr ? "-t" : "-r", TTCP_ConnSockID[i]));
                    }
                }
            }
            ...
            for (i = 0;  i < TTCP_NumConns; i++) {
                if (TTCP_ConnSockID[i] < 0) {
                    TTCP_Abort = DEF_YES;
                    TTCP_TRACE_INFO(("ttcp: Bad Socket Detected - Aborting.\r\n"));
                    break;
                }
            }
        }
        ...
        if (TTCP_Abort != DEF_YES) {                            /* Proceed with the transmission/reception of the test  */
            ...
            if (TTCP_Cli_Srvr) {                                /* Close all used sockets.                              */
                for (i = 0; i < TTCP_NumConns; i++) {
                    if (TTCP_ConnSockID[i] > -1) {
                        NetSock_Close(TTCP_ConnSockID[i], &TTCP_Err);
                        TTCP_TRACE_INFO(("ttcp%s: Client socket %d closed\r\n",
                                          TTCP_Cli_Srvr ? "-t" : "-r", TTCP_ConnSockID[i]));
                    }
                }
            } else {
                ...
                if (!TTCP_Transport) {
                    TTCP_TRACE_INFO(("ttcp%s: Listen socket %d closed\r\n",
            ...
            TTCP_msPerCall   = (DEF_TIME_NBR_mS_PER_SEC * TTCP_PerfTimerSecs) / (CPU_FP64)TTCP_NumCalls;
            TTCP_CallsPerSec = (CPU_FP64)TTCP_NumCalls  / TTCP_PerfTimerSecs;

            TTCP_OutFmt(TTCP_BytesPerSec);

                                                                /* Outputs test results.                                */
            TTCP_TRACE_INFO(("ttcp%s: %u bytes in %u.%u real sec = %s/sec (%u.%u bps)",
                              TTCP_Cli_Srvr ? "-t" : "-r",
            ...
            TTCP_TRACE_INFO(("ttcp%s: %d I/O calls, msec/call = %u.%u, calls/sec = %u.%u\r\n",
                              TTCP_Cli_Srvr ? "-t" : "-r",
            ...
            TTCP_TRACE_INFO(("\r\n"));
            TTCP_TRACE_INFO(("ttcp%s: %ld Rcv Timeouts, %d Rcv Errs %d Tx Errs, %d Unknown TX Rets\n\r",
                              TTCP_Cli_Srvr ? "-t" : "-r",
...
static  NET_SOCK_ID  TTCP_TaskTCP_Init (void)
{
    ...
    if (TTCPTaskTCP_SockID < 0) {
        TTCP_TRACE_INFO(("ttcp%s: Could not open a socket\r\n",
                          TTCP_Cli_Srvr ? "-t" : "-r"));
        return (TTCPTaskTCP_SockID);                            /* Could not open a socket.                             */
    } else {
        TTCP_TRACE_INFO(("ttcp%s: Listening socket opened = %d for PORT: %d\r\n",
    ...
    if (bind_status != NET_SOCK_BSD_ERR_NONE) {
        ...
        TTCP_TRACE_INFO(("ttcp%s: Could not bind to the TCP port\r\n",
                          TTCP_Cli_Srvr ? "-t" : "-r"));
        return (-1);
    }

                                                                /* Set blocking timeout until we 'connect'.             */
    NetOS_Sock_ConnAcceptTimeoutSet(TTCPTaskTCP_SockID, 100, &err);

    TTCPTaskTCP_Q = 5;                                          /* Reserve 5 buffers for every scoket created from this */
                                                                /* listening socket.                                    */
    ...
    if (listen_status != NET_SOCK_BSD_ERR_NONE) {
        ...
        TTCP_TRACE_INFO(("ttcp%s: Could not open listening socket\r\n",
                          TTCP_Cli_Srvr ? "-t" : "-r"));
        return (-1);
    }

    for (i = 0; i < NET_TCP_CFG_NBR_CONN; i++) {
        NetOS_TCP_RxQ_TimeoutSet(i, 100, &err);
    }
...
static  NET_SOCK_ID  TTCP_TaskUDP_Init (void)
{
    ...
    if (TTCPTaskUDP_SockID < 0) {
        ...
        TTCP_TRACE_INFO(("ttcp%s: UDP socket %d opened \r\n",
    ...
    if (bind_status != NET_SOCK_BSD_ERR_NONE) {
        ...
        TTCP_TRACE_INFO(("ttcp%s: Could not bind to the UDP port\r\n",
                          TTCP_Cli_Srvr ? "-t" : "-r"));
        return (-1);
    }
...
static  CPU_BOOLEAN  TTCP_Parse (CPU_CHAR  *TTCPCmdLine)
{
    ...
    while (TTCP_Strptr < TTCP_StrLen) {                         /* Remove spaces from the command line.                 */
        ...
        while ((TTCPCmdLine[TTCP_Strptr] != ' ') &
               (TTCP_Strptr < TTCP_StrLen)) {                   /* Extract the parameter until next space character or  */
                                                                /* end of string.                                       */
        ...
        if (TTCPPar[0] == '-') {
            switch (TTCPPar[1]) {                               /* Determine which parameter.                           */
                ...
                case 'd':                                       /* -d      set SO_DEBUG socket option - not supported.  */
                     TTCP_TRACE_INFO(("ttcp: -d option ignored: SOCKET options not supported\r\n"));
                     break;

                case 'D':                                       /* -D      don't buffer TCP writes (sets TCP_NODELAY    */
                                                                /*         socket option).                              */
                     TTCP_TRACE_INFO(("ttcp: -D option ignored: TCP_NODELAY option not supported\r\n"));
                     break;
                ...
                case 'b':                                       /*  -b ##  set socket buffer size (if supported).       */
                     TTCP_TRACE_INFO(("ttcp: -b option ignored: SOCKET buffer size option not supported\r\n"));
                     break;
                ...
                case 'T':                                       /* -T   Read data on reception to add processing time   */
                                                                /*      to the test.                                    */
                     TTCP_TouchData = 1;
                     TTCP_TRACE_INFO(("ttcp%s: Touch Data. Data processed as received\r\n",
                                       TTCP_Cli_Srvr?"-t":"-r"));
                     break;

                case 'L':
                     TTCP_LoopMode = 1;
                     TTCP_TRACE_INFO(("LOOP mode ACTIVATED.\r\n"));
                     break;
    ...
    if (TTCP_StrErr) {                                          /* On parsing error, outputs the command line menu.     */
                                                                /* Defintion of the -h option of the TTCP commande      */
                                                                /* line input.                                          */
        TTCP_TRACE_INFO(("Usage: -t [-options] host\r\n"));
        TTCP_TRACE_INFO(("       -r [-options]\r\n"));
        TTCP_TRACE_INFO(("Common options: \r\n"));
        TTCP_TRACE_INFO(("    -l ##   length of buffers read from or written to network (default 8192)\r\n"));
        TTCP_TRACE_INFO(("    -u      use UDP instead of TCP\r\n"));
        TTCP_TRACE_INFO(("    -p ##   port number to send to or listen at (default 5001)\r\n"));
        TTCP_TRACE_INFO(("    -s      disable sinkmode (enabled by default)\r\n"));
        TTCP_TRACE_INFO(("              sinmode enabled:\r\n"));
        TTCP_TRACE_INFO(("                -t: source (transmit) a pattern to network\r\n"));
        TTCP_TRACE_INFO(("                -r: sink (discard) all data from network\r\n"));
        TTCP_TRACE_INFO(("              sinmode disabled:\r\n"));
        TTCP_TRACE_INFO(("                -t: take data to be transmitted from serial port\r\n"));
        TTCP_TRACE_INFO(("                -r: write data received to the serial port\r\n"));
        TTCP_TRACE_INFO(("    -b ##   set socket buffer size (not supported)\r\n"));
        TTCP_TRACE_INFO(("    -k ##   number of simultaneous connections\r\n"));
        TTCP_TRACE_INFO(("    -f X    format for rate: k,K = kilo{bit,byte}; m,M = mega; g,G = giga\r\n"));
        TTCP_TRACE_INFO(("    -L      enable LOOP mode. Infinite loop the current operation\r\n"));
        TTCP_TRACE_INFO(("Options specific to -t:\r\n"));
        TTCP_TRACE_INFO(("    -n ##   number of source buffers written to network (default 2048)\r\n"));
        TTCP_TRACE_INFO(("    -D      don't buffer TCP writes (sets TCP_NODELAY socket option)\r\n"));
        TTCP_TRACE_INFO(("Options specific to -r:\r\n"));
        TTCP_TRACE_INFO(("    -B      for -s, only output full blocks as specified by -l\r\n"));
        TTCP_TRACE_INFO(("    -T      \"touch\": access each byte as it is read\r\n\n"));
    }
    return (TTCP_StrErr);
}
        ...
        switch (err) {                                          /* Socket error handling.                               */
            case NET_SOCK_ERR_NONE:
                 ...
                 if (TTCP_NumIntvl[ConnIdx] > 1000000) {
                     TTCP_TRACE_INFO(("Stream %d: Bytes Sent So Far: %11.0f\n\r", 
            ...
            case NET_SOCK_ERR_FAULT:
                 TTCP_TRACE_INFO(("ttcp%s: Closing socket %d Fatal socket error number %d.\r\n",
            ...
            for (ConnIdx = 0; ConnIdx < TTCP_NumConns; ConnIdx++) {
                ...
                while (count < TTCP_BufLen) {
                    ...
                    if (data_received > 0) {
                        ...
                        if (TTCP_NumIntvl[ConnIdx] > 1000000) {
                            TTCP_TRACE ("Stream %d: Bytes RCVD So Far: %11.0f\n\r", 
                                ConnIdx, TTCP_NumBytesStream[ConnIdx]);
                        ...
                        if (TTCP_SinkMode == 0) {
                            while (print_count < count) {
                                ...
                                TTCP_TRACE_INFO(("%c", TTCP_Buf[print_count]));
                                print_count++;
                            }
                        }
                        ...
                        TTCP_TRACE("Socket Closed Received\r\n");
                        return;
                    }
                }        /* While loop */
            }            /* For Loop */
                ...
                if (NumReceived[ConnIdx] < total) {
                    ...
                    if (data_received > 0) {
                        ...
                        if (TTCP_SinkMode == 0) {               /* Not Sink Mode => output data to serial port.         */
                            ...
                            TTCP_TRACE_INFO(("%s", TTCP_Buf));  /* Output the string to the serial port.                */
                        }
                        ...
                        if (err == NET_SOCK_ERR_RX_Q_EMPTY) {
                            ...
                            TTCP_TRACE_INFO(("Stream %d: Invalid Type, reducing activeConns\r\n", ConnIdx));
                            ActiveConns--;
                        } else {
                            TTCP_NumRcvErrs ++;
                        }
                    } else if (data_received == 0) {
                        TTCP_TRACE("Stream %d: Close socket detected\n\r",
                                    ConnIdx);
                        return;
                    }
                }
