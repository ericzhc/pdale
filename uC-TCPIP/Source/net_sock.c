...
*                               NET_CONN_ERR_NOT_USED                   Connection NOT currently used.
...
*               called by application function(s).
*
* Note(s)     : (2) NetSock_Conn() blocked until network initialization completes; perform NO action.
*
*               (3) (a) Socket address structure 'Family' member MUST be configured in host-order & MUST
*                       NOT be converted to/from network-order.
*
...
*                               NET_SOCK_ERR_CONN_SIGNAL        Socket connection signal failed.
*
* Return(s)   : none.
*
...
*               application function(s).
*
* Note(s)     : (1) NetSock_ConnSignalReq() blocked until network initialization completes; perform NO action.
*
*               (2) See 'NetSock_ConnHandlerStream()  Note #2c3'.
*********************************************************************************************************
*/
...
/*$PAGE*/
    ...
    switch (psock->State) {
        ...
        case NET_SOCK_STATE_BOUND:
        case NET_SOCK_STATE_LISTEN:                             /* See Note #2.                                         */
        case NET_SOCK_STATE_CONN_IN_PROGRESS:
             break;
...
*
*                                                                   -- RETURNED BY NetOS_Sock_ConnAcceptSignal() : ---
*                               NET_SOCK_ERR_CONN_SIGNAL            Socket connection accept signal failed.
*
* Return(s)   : none.
*
* Caller(s)   : NetTCP_RxPktConnHandlerSignalConn().
...
*               application function(s).
*
* Note(s)     : (1) NetSock_ConnSignalAccept() blocked until network initialization completes; perform NO action.
*
*               (2) On any faults, network connection NOT freed/closed; caller function(s) handle fault condition(s).
*********************************************************************************************************
*/
...
/*$PAGE*/
    ...
    NetOS_Sock_ConnAcceptSignal(sock_id, perr);                 /* Signal sock accept.                                  */
    if (*perr != NET_SOCK_ERR_NONE) {
...
*               application function(s).
*
* Note(s)     : (1) NetSock_ConnSignalClose() blocked until network initialization completes; perform NO action.
*
...
*                   (b) Acquire   network   lock
*                   (c) Validate  socket
*                   (d) Validate  socket    connection state
*                   (e) Configure transport connection
...
*               called by application function(s).
*
* Note(s)     : (2) NetSock_Listen() blocked until network initialization completes; perform NO action.
*
...
/*$PAGE*/
                                                                /* -------------- VALIDATE SOCK PROTOCOL -------------- */
    switch (psock->SockType) {                                  /* Validate sock for stream protocol(s) [see Note #3].  */
        case NET_SOCK_TYPE_DATAGRAM:
             NET_CTR_ERR_INC(NetSock_ErrInvalidSockTypeCtr);
             NetOS_Unlock();
            *perr =  NET_SOCK_ERR_INVALID_TYPE;
...
*               application function(s).
*
* Note(s)     : (2) NetSock_ListenQ_IsAvail() blocked until network initialization completes; perform NO action.
*
...
/*$PAGE*/
    ...
    q_avail = NetSock_ConnAcceptQ_IsAvail(psock, perr);         /* Chk if listen Q avail for new conns (see Note #3).   */
...
*                                          NetSock_Accept()
*
* Description : (1) Get a new socket accepted from a listen socket :
*
*                   (a) Validate address pointers
*                   (b) Validate listen  socket handle identifier
*                   (c) Acquire  network lock
*                   (d) Validate listen socket :
*                       (1) Validate listen socket type
*                       (2) Validate listen socket connection state
*                   (e) Wait on  listen socket's connection accept queue
*                   (f) Configure accept socket :
...
*                   (g) Return accept socket connection's remote address
*                   (h) Release network lock
*                   (i) Return  socket descriptor/handle identifier of new accept socket
*                         OR
...
*                               NET_SOCK_ERR_NONE_AVAIL                 NO available sockets to allocate.
*
*                                                                       - RETURNED BY NetOS_Sock_ConnAcceptWait() : -
*                               NET_SOCK_ERR_CONN_SIGNAL_TIMEOUT        Socket connection accept queue empty.
*
*                                                                       -------- RETURNED BY NetOS_Lock() : ---------
*                               NET_OS_ERR_LOCK                         Network access NOT acquired.
*
...
*               called by application function(s).
*$PAGE*
* Note(s)     : (2) NetSock_Accept() blocked until network initialization completes; perform NO action.
*
...
/*$PAGE*/
   ...
   *paddr_len =  0;                                             /* Init len for err     (see Note #4a2).                */
    ...
    used         =  DEF_BIT_IS_SET(psock_listen->Flags, NET_SOCK_FLAG_USED);
    if (used != DEF_YES) {
        NET_CTR_ERR_INC(NetSock_ErrNotUsedCtr);
        NetOS_Unlock();
       *perr =  NET_SOCK_ERR_NOT_USED;
...
/*$PAGE*/
                                                                /* ---------- VALIDATE LISTEN SOCK PROTOCOL ----------- */
    switch (psock_listen->SockType) {                           /* Validate sock for stream protocol(s) [see Note #6].  */
        case NET_SOCK_TYPE_DATAGRAM:
             NET_CTR_ERR_INC(NetSock_ErrInvalidSockTypeCtr);
             NetOS_Unlock();
            *perr =  NET_SOCK_ERR_INVALID_TYPE;
...
/*$PAGE*/
    ...
    NetOS_Unlock();
    NetOS_Sock_ConnAcceptWait(sock_id, perr);
    NetOS_Lock(&err);
    if (  err != NET_OS_ERR_NONE) {
                                                                /* #### Close sock/Reset sock conn?                     */
    ...
    if (*perr != NET_SOCK_ERR_NONE) {
                                                                /* #### Close sock/Reset sock conn?                     */
         NetOS_Unlock();
         return (NET_SOCK_BSD_ERR_ACCEPT);                      /* Rtn err from NetOS_Sock_ConnAcceptWait().            */
    }
...
/*$PAGE*/
   ...
   (void)NetConn_ID_AppSet((NET_CONN_ID) conn_id_accept,
   ...
   *perr =  NET_SOCK_ERR_INVALID_FAMILY;
    ...
    if (*perr != NET_SOCK_ERR_NONE) {                           /* See Note #9c.                                        */
         NetSock_CloseHandler(psock_accept, DEF_YES, DEF_YES);
         NetOS_Unlock();
         return (NET_SOCK_BSD_ERR_ACCEPT);
    }
...
*
*                   (a) Validate   receive address buffer                           See Notes #4 & 5
...
*
*                   See also 'NetSock_RxDataHandler()  Note #3'.
*
*               (3) #### Only some socket receive flag options are implemented.  If other flag options are 
...
*                   See also Note #9.
*$PAGE*
*               (7) IEEE Std 1003.1, 2004 Edition, Section 'recvfrom() : RETURN VALUE' states that :
*
...
*
*                   See also 'NetSock_RxDataHandler()  Note #8'.
*
*               (8) Pointers to variables that return length or size values MUST be initialized to return
*                   zero PRIOR to all other validation or function handling in case of any error(s).
*
*               (9) #### IP options arguments may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
                                       ...
                                       NET_ERR            *perr)
{
    ...
    if (paddr_len == (NET_SOCK_ADDR_LEN *)0) {                  /* See Note #5b.                                        */
        NET_CTR_ERR_INC(NetSock_ErrNullPtrCtr);
       *perr =  NET_SOCK_ERR_NULL_PTR;
        return (NET_SOCK_BSD_ERR_RX);
    }

     addr_len = *paddr_len;                                     /* Save     initial addr len         (see Note #5a1A).  */
#endif
   *paddr_len =  0;                                             /* Cfg      dflt    addr len for err (see Note #5a2).   */

#if (NET_ERR_CFG_ARG_CHK_EXT_EN == DEF_ENABLED)
    if (addr_len < NET_SOCK_ADDR_SIZE) {                        /* Validate initial addr len         (see Note #5a1B).  */
        NET_CTR_ERR_INC(NetSock_ErrInvalidAddrLenCtr);
       *perr =  NET_SOCK_ERR_INVALID_ADDR_LEN;
        return (NET_SOCK_BSD_ERR_RX);
    }
...
*
*               NET_SOCK_BSD_RTN_CODE_CONN_CLOSED,       if socket connection closed (see Note #4b).
*
*               NET_SOCK_BSD_ERR_RX,                     otherwise                   (see Note #4c1).
*
* Caller(s)   : recv().
*
*               This function is a network protocol suite application interface (API) function & MAY be 
...
*                   so that requested flag options are NOT silently ignored.
*
*               (4) IEEE Std 1003.1, 2004 Edition, Section 'recv() : RETURN VALUE' states that :
*
...
*
*                   See also 'NetSock_TxDataHandler()  Note #3'.
*
...
*                   See also 'net_sock.h  NETWORK SOCKET ADDRESS DATA TYPES  Note #2'.
*
*               (5) (a) IEEE Std 1003.1, 2004 Edition, Section 'sendto() : RETURN VALUE' states that :
*
...
*
*                   See also 'NetSock_TxDataHandler()  Note #6'.
**********************************************************************************************************
*/
/*$PAGE*/
...
*                               NET_SOCK_ERR_NONE                   Socket data successfully transmitted &/or prepared 
*                                                                       for transmission; check return value for number 
*                                                                       of data octets transmitted (see Note #4a1).
*
*                               NET_ERR_INIT_INCOMPLETE             Network initialization NOT complete.
*                               NET_SOCK_ERR_NULL_PTR               Argument 'p_data' passed a NULL pointer.
*                               NET_SOCK_ERR_NOT_USED               Socket NOT currently used.
...
*                               NET_OS_ERR_LOCK                     Network access NOT acquired.
*
* Return(s)   : Number of positive data octets transmitted, if NO errors                (see Note #4a1).
*
*               NET_SOCK_BSD_RTN_CODE_CONN_CLOSED,          if socket connection closed (see Note #4b).
*
*               NET_SOCK_BSD_ERR_TX,                        otherwise                   (see Note #4a2A).
*
* Caller(s)   : send().
*
*               This function is a network protocol suite application interface (API) function & MAY be 
...
*                   so that requested flag options are NOT silently ignored.
*
*               (4) (a) IEEE Std 1003.1, 2004 Edition, Section 'send() : RETURN VALUE' states that :
*
...
/*$PAGE*/
   ...
   *perr =  NET_SOCK_ERR_INVALID_FAMILY;                        /* Rtn net sock err ...                                 */
    ...
    if (*perr != NET_SOCK_ERR_NONE) {                           /* See Note #2a2.                                       */
         NetSock_CloseSockFromClose(psock);
...
/*$PAGE*/
   ...
   (void)NetConn_AddrLocalSet((NET_CONN_ID      ) conn_id,      /* Set sock's local addr.                               */
                              (CPU_INT08U      *)&addr_local[0],
                              (NET_CONN_ADDR_LEN) NET_SOCK_CFG_ADDR_LEN,
                              (CPU_BOOLEAN      ) addr_over_wr,
                              (NET_ERR         *) perr);
...
*                               NET_SOCK_ERR_CONN_SIGNAL_TIMEOUT    Socket receive queue empty.
*
*                                                                   -------------- RETURNED BY NetOS_Lock() : ---------------
*                               NET_OS_ERR_LOCK                     Network access NOT acquired.
*
...
/*$PAGE*/
    ...
    if (*perr != NET_SOCK_ERR_NONE) {
         NetSock_CloseSock(psock, DEF_YES, DEF_YES);
         return (NET_SOCK_BSD_ERR_CONN);                        /* Rtn err from NetOS_Sock_ConnReqWait().               */
    }
...
#endif
                                                                            /* ---------- VALIDATE NBR USED ----------- */
                                                                            /* Chk for sock max conn accept lim.        */
...
* Description : (1) Receive data from a socket :
*
*                   (a) Validate receive data buffer                                    See Note #3
*                   (b) Validate receive flags                                          See Note #4
*                   (c) Validate socket  handle identifier
*                   (d) Acquire  network lock
*                   (e) Validate socket
*                   (f) Receive  socket  data
*                   (g) Release  network lock
*
*
* Argument(s) : sock_id             Socket descriptor/handle identifier of socket to receive data.
...
*               NetSock_RxData().
*
* Note(s)     : (2) NetSock_RxDataHandler() blocked until network initialization completes; perform NO action.
*
...
*
*               (4) #### Only some socket receive flag options are implemented.  If other flag options 
...
*                   See also Note #11.
*$PAGE*
*               (8) IEEE Std 1003.1, 2004 Edition, Section 'recv() : RETURN VALUE' states that :
*
*                   (a) "Upon successful completion, recv() shall return the length of the message in bytes."
*
*                   (b) "If no messages are available to be received and the peer has performed an orderly
...
*                   included as an extra precaution in case 'SockType' is incorrectly modified.
*
*              (11) #### IP options arguments may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
                                                  ...
                                                  NET_ERR            *perr)
{
    ...
    flag_mask = NET_SOCK_FLAG_NONE         |
                ...
                NET_SOCK_FLAG_RX_NO_BLOCK;
                                                                /* If any invalid flags req'd, rtn err (see Note #4).   */
...
*                       error is returned.
*
*                   See also 'NetSock_RxDataHandler()  Note #3a'.
*
...
/*$PAGE*/
    ...
    if (*perr != NET_SOCK_ERR_NONE) {
         return (NET_SOCK_BSD_ERR_RX);                          /* Rtn err from NetOS_Sock_RxQ_Wait().                  */
    }
...
*                       stream-type socket.
*
*                   See also 'NetSock_RxDataHandler()  Note #3b'.
*
*               (3) If global socket blocking ('NET_SOCK_CFG_BLOCK_SEL') configured for blocking operation 
*                   ('NET_SOCK_BLOCK_SEL_BLOCK' or 'NET_SOCK_BLOCK_SEL_DFLT') but 'flags' argument set to 
...
* Description : (1) Transmit data through a socket :
*
*                   (a) Validate transmit data                                          See Note #3
*                   (b) Validate transmit flags                                         See Note #4
*                   (c) Validate socket   handle identifier
*                   (d) Acquire  network  lock
*                   (e) Validate socket
*                   (f) Validate remote   address                                       See Note #5
*                   (g) Transmit socket   data
*                   (h) Release  network  lock
*
*
* Argument(s) : sock_id         Socket descriptor/handle identifier of socket to transmit data.
...
* Caller(s)   : sendto().
*
* Note(s)     : (2) NetSock_TxDataHandler() blocked until network initialization completes; perform NO action.
*
*               (3) (a) (1) (A) Datagram-type sockets transmit & receive all data atomically -- i.e. every
*                               single, complete datagram transmitted MUST be received as a single, complete
*                               datagram.  Thus, each call to transmit data MUST be transmitted in a single, 
                                                  ...
                                                  NET_ERR            *perr)
{
    ...
    if (data_len < 1) {                                         /* Validate data len (see Note #3b).                    */
...
*                   (b) 'data_len' of 0 octets NOT allowed.
*
*                   See also 'NetSock_TxDataHandler()  Note #3a'.
*
*               (3) If global socket blocking ('NET_SOCK_CFG_BLOCK_SEL') configured for blocking operation 
*                   ('NET_SOCK_BLOCK_SEL_BLOCK' or 'NET_SOCK_BLOCK_SEL_DFLT') but 'flags' argument set to 
                                               ...
                                               NET_ERR   *perr)
{
   ...
   *perr =  NET_SOCK_ERR_INVALID_FAMILY;
   ...
   (void)NetConn_ID_TransportSet(conn_id, conn_id_transport, perr);
    if (*perr != NET_CONN_ERR_NONE) {
         conn_err = DEF_YES;
    }
...
*                               NET_SOCK_ERR_CONN_CLR           Socket connection signal clear failed.
*                               NET_SOCK_ERR_RX_Q_CLR           Socket receive    queue  clear failed.
*
* Return(s)   : none.
*
...
*                                                               - RETURNED BY NetOS_Sock_ConnAcceptClr() : -
                                   ...
                                   NET_ERR   *perr)
{
    ...
    NetOS_Sock_ConnAcceptClr(psock->ID, &err);
    if (err != NET_SOCK_ERR_NONE) {                             /* If  sock conn accept NOT clr'd, ...                  */
        discard     = DEF_YES;                                  /* ... discard sock (see Note #3b).                     */
        err_discard = err;
    }
...
* Caller(s)   : NetSock_BindHandler().
*
* Note(s)     : (2) #### 'perr' may NOT be necessary (remove if unnecessary).
*********************************************************************************************************
*/
/*$PAGE*/
static  NET_PORT_NBR  NetSock_RandomPortNbrGet (NET_ERR  *perr)
